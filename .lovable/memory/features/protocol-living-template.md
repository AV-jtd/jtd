...
4. **Преемственность между протоколами** обеспечивается тем же механизмом, что и для `cross_functional`: при создании нового living-протокола можно перенести открытые задачи из предыдущих (диалог `NewProtocolDialog` — это поведение зашито на `isCrossFunctional`, для living в будущем можно расширить — сейчас не подключено).
