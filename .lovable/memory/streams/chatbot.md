...
## Статус (2026-04-19): СТРИМ НА ПАУЗЕ

Артём поставил стрим на паузу после анализа использования.

### Данные использования (за 30 дней до 2026-04-19)
- 8 уникальных пользователей AI-ассистента, 27 бесед в `assistant` + 5 в `project_chat` = 32.
- За **последние 7 дней — всего 2 активных** (Владимир ITech 8 бесед, Мария Дудорова 3).
- Артём (владелец) последний раз 28 марта.
- **80% запросов — preset-кнопки** из AiAssistant («Покажи просроченные», «Анализ сроков», «Критический путь», «Статус-отчёт», «Обзор проектов», «Рекомендации по продуктивности»).
- Свободный ввод: **2 из ~30** («как отключить ночную тему», «можешь внести 10 задач»).
- Точечные AI (Risk Radar, fill_section) в `ai_conversations` не пишутся — отдельно не оценить.

### Что НЕ делаем (и почему)
- **Этап 2 — Registry** ❌ — рефакторинг 400 строк ради 2 активных юзеров не окупается.
- **Этап 5 — inline-actions TG** ❌ — нарушает GTD «кинул-забыл».
- **Этап 6 — Voice web** ❌ — preset-кнопка быстрее голоса на десктопе, голос уже есть в TG.
- **Утренний дайджест** ❌ — инфра готова (`send-weekly-report` уже daily-ready), но Артём не хочет.

### Реальные точки роста (НЕ начинать без явного запроса)
1. **Кеш preset-промптов** на 1-2ч — у Владимира 8 одинаковых запросов подряд за неделю.
2. **Расширить набор preset-кнопок** — раз свободным вводом не пользуются.
3. **«Сохранить ответ в Wiki»** — превращать AI-ответы в артефакты проекта.

### Когда вернуться
- Если активных юзеров AI станет ≥10/неделю.
- Если появится конкретная боль («долго ждать ответ», «дублируются запросы»).
- Если Артём сам захочет вернуться — НЕ предлагать проактивно.

### Что унифицировано
- `src/lib/quickTaskParse.ts` — общий парсер `@имя`, `+Nд`, `до DD.MM`, `!`, `#тег` (web + TG hints).
- Используется: TaskCreateBar, QuickCreateForm, BulkTaskDialog, AiAssistant, telegram-webhook.
