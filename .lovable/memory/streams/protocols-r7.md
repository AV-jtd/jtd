...
- **Этап 5 ✅ Soft-review workflow** — При работе с черновиком автоматически создаётся мягкая задача `task_type='protocol_review'` («Изучить, доработать протокол: <название>», срок завтра 18:00) у автора и каждого ответственного из строк черновика. Реализовано БД-триггером `trg_protocol_draft_assignee_review` на `tasks` + функцией `ensure_protocol_review_task` (SECURITY DEFINER). Уникальный частичный индекс `uniq_protocol_review_per_user` на `(source_protocol_id, assigned_to) WHERE task_type='protocol_review'` гарантирует не более одной задачи на (протокол, пользователь). Эти задачи **видимы** в Inbox/«Все задачи» (НЕ is_draft) — в отличие от самих строк черновика, которые скрыты из глобального списка через фильтр `is_draft=false` в `useTasks` (применяется только когда `groupId` не задан). При публикации (`usePublishProtocol`) review-задачи авто-закрываются с `closure_result='Опубликовано'`; при удалении черновика (`useDiscardProtocolDraft`) — `closure_result='Черновик удалён'` (закрытие до DELETE группы, т.к. у них `source_protocol_id`, а не `group_id`, каскад их не трогает). Публикацию **не блокируют** — это только напоминание/информирование, не workflow утверждения.
- **Этап 6 ✅ Внешняя/внутренняя публикация + стороны + двухконтурная модель** — большой блок работы 2026-04-19 (см. отдельные feature-файлы `protocol-*.md`):
  - **Парсинг сторон из названия** (`protocolSides.ts`): «Лента x Дороничи» → партнёр/наша сторона. Авто-привязка партнёра к CRM-клиенту, диалог быстрого создания клиента.
  - **Карточки сторон в шапке** вместо «Внутренние/Внешние участники»: 3 автономных слота логотипов (иконка протокола, our_logo_url per-protocol с дефолтом «Дороничи», clients.logo_url партнёра), кастомные участники партнёра inline-чипами.
  - **AssigneePicker** переписан: компании партнёра (uniq orgs ∪ linkedClient ∪ parsedPartner) + контактные лица партнёра (external_attendees ∪ CRM contact) + наши пользователи. Назначение на компанию: `external_assignee.role='company'`. Бейдж `CRM` для контактов из карточки клиента.
  - **Внутренний контур (двухконтурная модель)** — `protocol_scope='internal'` для самостоятельных внутренних задач + `status_meta.linked_project_id` / `linked_stream_key` для внутреннего контекста внешней строки (одна задача — два контекста: физически в протоколе, логически в NPD/PMO/CRM-проекте). Триггер `sync_linked_project_participants` авто-добавляет владельца linked-проекта и автора задачи в `task_participants`. UI: красный border-l, иконка Lock, чип «не уходит партнёру». Места: `ProtocolInternalSection`, `ExternalRowInternalLayer`. Видимость linked-задач в `NpdSwimlaneMatrix` через два запроса с дедупом.
  - **Тема как проект** (`TopicCell` + `useCreateEventTopic`): тема = тег категории `event_topic`, при создании авто-привязывается к одноимённому проекту (`task_groups.linked_tag_id`), отображается с иконкой 📁.
  - **Тумблер «Имена / Стороны»** в `ProtocolPreviewDialog`: сегментированный переключатель в шапке, влияет одновременно на PDF, Email-резюме, Telegram. Сторона определяется по `profiles.organization` (пусто = наша сторона).
  - **Черновик → Mission Control → Публикация**: `task_groups.draft_status` + `tasks.is_draft`, импорт всегда `asDraft=true`, баннер с действиями «Опубликовать / Удалить черновик / Превью», публикация = bulk UPDATE без массовых уведомлений (MVP). ⚠️ Инвариант: `useTasks(protocolId)` обязан вызываться с argom внутри `/protocols/:id`, иначе черновики скрыты глобальным фильтром `is_draft=false`.
  - **Мобильная адаптация** (`ProtocolMobileRow` + sticky FAB): на <md таблица заменяется вертикальным стеком карточек, плавающая `+` 56px primary с safe-area, тап по карточке → Bottom Sheet с полным `TaskItem`. Внутренний красный блок свёрнут по умолчанию на мобиле. Шапка: title `text-lg break-words`, флекс-врап для бейджа «Черновик», draft-bar компактен (icon-only кнопки). 6 смарт-фильтров в сетке `grid-cols-2 sm:grid-cols-3 lg:grid-cols-6` (добавлен «Без срока» с иконкой `CalendarOff`).
  - **Превью унифицирован с рабочей таблицей**: description в превью/PDF теперь `text-xs text-neutral-500 line-clamp-2 leading-[1.4]`, title `leading-snug`.
