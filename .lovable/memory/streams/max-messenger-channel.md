...
## Этап 4 — Единый код привязки (вариант 2, реализовано)
- **Решение**: один универсальный код на проект. Код НЕ удаляется после первой привязки — `linkGroupChat` оставляет токен живым до `expires_at`, поэтому тем же кодом делаешь `/link КОД` и в TG, и в MAX. Сообщение-подтверждение подсказывает про второй канал.
- **UI**: `src/components/ChatLinkDialog.tsx` (кнопка Link2 в шапке ProjectChat, embedded и full). Генерит код channel=`any`, expires +24ч, чистит прежние коды проекта (delete by group_id+created_by), показывает статус привязки TG/MAX.
- **send-chat-telegram**: если у проекта заполнен `telegram_group_chat_id`/`max_group_chat_id` → постит через `fanOutToGroups` (origin=`web`) и НЕ шлёт per-user DM. Непривязанные проекты — прежняя DM-логика по opt-in.
- **telegram-webhook**: `/link` теперь сначала пробует распознать аргумент как КОД (regex + наличие токена в chat_link_tokens). Если код найден → `linkGroupChat` (пишет task_groups.telegram_group_chat_id) + upsert legacy `telegram_group_chats` (чтобы /task /spisok /done не сломались). Иначе фолбэк на старый `/link Название проекта`.
- **Две системы привязки TG**: legacy `telegram_group_chats` (telegram_chat_id→group_id, питает командные /task /spisok) и new `task_groups.telegram_group_chat_id` (питает fan-out единого чата). Код-флоу пишет в ОБЕ. MAX живёт целиком на новой системе.
