...
- **Протоколы/встречи** (два источника, объединяются и дедуплицируются по id):
  1. **Прямая привязка** — `task_groups` где `project_type='protocol'` и `protocol_meta->>client_id = clientId`. Протокол появляется на карточке сразу, как только в нём указан клиент (даже если у задач протокола `client_id` не проставлен). Задачи таких протоколов тоже подтягиваются в «Ключевые задачи» (по `tasks.group_id IN (директ-протоколы)`, кроме `task_type='protocol_review'`).
  2. **Производная** — `task_groups` по `tasks.source_protocol_id` задач клиента.
  - Причина: раньше карточка смотрела только на (2), а у задач протоколов `client_id` обычно null → протокол не «подвязывался». Прямая привязка (1) закрывает это без изменения бизнес-логики задач.
