...
import { KmOpsTasks } from "./KmOpsTasks";
import { patchGroupInCache, restoreGroupSnapshots } from "../lib/kmCache";
import { kmTimeInStage, isKmProjectBlocked, isKmProjectStuck } from "../lib/kmAnalytics";

/**
 * Inline-editable KM meta chip (Сеть / Бренд / Проект / Дроп).
 * Click to edit → updates task_groups.km_meta[field] with optimistic cache patch,
...
function KmMetaChip({
  ...
  const save = useMutation({
    ...
    onMutate: async (text: string) => {
      const nextMeta: any = { ...(meta || {}) };
      if (text) nextMeta[field] = text; else delete nextMeta[field];
      const snapshots = patchGroupInCache(qc, group.id, { km_meta: nextMeta } as any);
      return { snapshots };
    },
 ...
 * - stage grid: each stage with index, status dot, title, assignee initials, due date
 * - clicking a stage cell expands its TaskItem inline (full edit: steps, comments, deadline, assignee)
 */
function KmExpandedRowInner({ project, stages, onOpenGantt, activeStageKey: controlledStageKey, onActiveStageChange }: Props) {
  ...
  const saveComment = useMutation({
    ...
    // Invalidation is handled by the optimistic patch in KmMatrixRow already;
  ...
  const { data: profileMap = new Map<string, string>() } = useQuery({
    queryKey: ["km-profiles", assigneeIds.sort().join(",")],
    queryFn: async () => {
  ...
  const timeInStage = kmTimeInStage(project);
  const blocked = isKmProjectBlocked(project);
  const stuck = isKmProjectStuck(project);
  ...
  return (
    <div className="bg-stm-bg/95 border-b border-stm-border/40 px-4 py-4 space-y-4">
      ...
      <header className="flex flex-wrap items-end justify-between gap-4 px-1">
        <div className="space-y-0.5 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 pt-1">
            <KmMetaChip group={group} meta={meta} field="retailer" label="Сеть" placeholder="X5, Лента…" />
            <KmMetaChip group={group} meta={meta} field="brand" label="Бренд" placeholder="Бережное томление…" />
            <KmMetaChip group={group} meta={meta} field="project" label="Проект" placeholder="Чистые составы…" />
            <KmMetaChip group={group} meta={meta} field="drop" label="Дроп" placeholder="Q2 2026…" />
          </div>
        </div>
      ...
      {/* SKU COMMENT — inline editable description */}
      <div className="px-1">
        {editingComment ? (
          <div className="flex items-start gap-2">
            ...
            <textarea
              ...
              placeholder="Комментарий по SKU: нюансы запуска, договорённости… (Esc — отмена, ⌘/Ctrl+Enter — сохранить)"
              rows={2}
      ...
      {/* MAIN PANEL — roadmap strip + stage cards */}
      ...
      <KmOpsTasks groupId={group.id} />
    </div>
  );
}

export const KmExpandedRow = React.memo(KmExpandedRowInner);
