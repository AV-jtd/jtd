...
import {
  useUpdateKmGroupMeta,
  useSetKmGroupManager,
  useAddKmGroupParticipants,
  type KmGroupField,
} from "../hooks/useKmProjects";
import { getInitials } from "@/lib/initials";

const FIELD_LABELS: Record<KmGroupField, string> = {
  retailer: "Сеть",
  brand: "Бренд",
  drop: "Дроп / контракт",
  project: "Проект",
...
export default function KmEditGroupDialog({
  ...
  const updateMeta = useUpdateKmGroupMeta();
  const setManager = useSetKmGroupManager();
  const addParticipants = useAddKmGroupParticipants();
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4 mt-1">
          {/* Name / merge */}
          <div>
            <Label htmlFor="km-grp-name" className="text-xs">Название ({FIELD_LABELS[field]})</Label>
            <Input
              id="km-grp-name"
              value={name}
