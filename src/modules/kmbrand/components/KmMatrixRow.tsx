...
import { KmMatrixCell } from "./KmMatrixCell";
...
import { KmExpandedRow } from "./KmExpandedRow";
import KmStatusControl from "./KmStatusControl";
import { getKmLifecycleOption } from "../lib/stages";
import { kmRowState, kmTimeInStage } from "../lib/kmAnalytics";
...
function KmMatrixRowInner({ project, stages, expanded, onToggleExpand, onOpenGantt, activeStageKey, onActiveStageChange, density = "comfortable" }: Props) {
  const { group, meta, currentStageKey, stageTasks, progress, archivedAt, archiveComment, lifecycle } = project;
  const isArchived = !!archivedAt;
  const lifecycleOpt = getKmLifecycleOption(lifecycle);

  // Row state drives the left status strip + progress bar color.
  const state = kmRowState(project);
  ...
  // Inline-editable SKU comment. Same field as in KmExpandedRow → changes
          ...
          return (
            ...
            >
              <KmMatrixCell
                ...
                isCurrent={stage.key === currentStageKey}
                milestone={stage.milestone}
                milestoneLabel={stage.milestone ? stage.short : undefined}
                groupId={group.id}
                stageKey={stage.key}
                stageTitle={stage.title}
              />
...
export const KmMatrixRow = React.memo(KmMatrixRowInner);
