...
import { patchGroupInCache, restoreGroupSnapshots, KM_KEYS } from "../lib/kmCache";
import {
  KM_LIFECYCLE,
  getKmLifecycleOption,
  type KmLifecycle,
  type KmMeta,
} from "../lib/stages";
 ...
 * - Only "Стоп от сети" requires a mandatory comment.
 * - Switching to a non-archiving status clears closed_at and the comment.
 */
export default function KmStatusControl({ groupId, groupName, meta, current, archivedAt, compact, fullWidth }: Props) {
  ...
  const currentOpt = getKmLifecycleOption(current);

  const mutation = useMutation({
    mutationFn: async (vars: { status: KmLifecycle; comment: string | null }) => {
      const opt = getKmLifecycleOption(vars.status);
    ...
    onMutate: async (vars) => {
      await qc.cancelQueries({ queryKey: KM_KEYS.groups() });
      const opt = getKmLifecycleOption(vars.status);
      const snapshots = patchGroupInCache(qc, groupId, {
        km_meta: { ...meta, lifecycle: vars.status },
        closed_at: opt.archives ? (archivedAt ?? new Date().toISOString()) : null,
        archive_comment: vars.status === "stop" ? vars.comment : null,
      } as any);
      return { snapshots };
  ...
  const handleSelect = (status: KmLifecycle) => {
    if (status === current) return;
    const opt = getKmLifecycleOption(status);
      ...
      <Dialog open={commentOpen} onOpenChange={setCommentOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-2">
            <Label htmlFor="km-stop-comment" className="text-xs">
