...
interface Props {
  project: StmProject;
  stages: StmStage[];
  onOpenSku?: (groupId: string) => void;
}

function StmMatrixRowInner({ project, stages, onOpenSku }: Props) {
  const { group, meta, currentStageKey, stageTasks, progress } = project;

  return (
    <div className="flex border-b border-stm-border/30 hover:bg-stm-glass/20 transition-colors">
      {/* Sticky SKU column */}
      <button
        type="button"
        onClick={() => onOpenSku?.(group.id)}
