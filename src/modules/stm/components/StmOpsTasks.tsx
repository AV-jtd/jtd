...
/**
 * Operational tasks block for an STM SKU card.
 * Two sections:
 ...
 *
 * Stage-tasks (task_type='stm_stage') are excluded — they live in the Chronograph above.
 * Inline create bar uses the same TaskCreateBar / TaskItem as the global "Все задачи" view
 * for a single, consistent workflow.
 */
function StmOpsTasksInner({ groupId }: Props) {
  ...
  const { data: tasks = [], isLoading } = useQuery({
    queryKey: ["stm-ops-tasks", groupId],
    enabled: !!groupId,
    staleTime: 30_000,
    queryFn: async () => {
  ...
  const { data: protocolMap = new Map<string, { name: string; icon: string | null }>() } = useQuery({
    queryKey: ["stm-ops-protocol-names", protocolIds.sort().join(",")],
...
export const StmOpsTasks = React.memo(StmOpsTasksInner);
export default StmOpsTasks;
