...
import { ArrowLeft, Settings, Loader2, Plus, AlertTriangle } from "lucide-react";
import ModuleLayout from "@/components/ModuleLayout";
import { Button } from "@/components/ui/button";
import { BoardColumn } from "@/components/board/BoardColumn";
import { DraggableWrapper } from "@/components/board/DraggableWrapper";
import { useBoardDnd } from "@/hooks/useBoardDnd";
import { useKanbanBoard, useMoveCard, useColumnMutations, type KanbanColumn as KanbanColumnT } from "@/hooks/useKanbanBoards";
...
export default function KanbanBoardPage() {
  ...
  // Fetch tasks: for project — group tasks; for personal — all user tasks.
  ...
  // For personal boards, show only tasks already placed on this board OR all uncompleted user tasks.
  // Simpler MVP: show all user/project tasks; first column is default for any without a position row.
  ...
  const cardsByColumn = useMemo(() => {
    ...
    // Sort: positioned first by position, others by task position/created_at
    for (const [cid, arr] of out) {
      arr.sort((a, b) => {
  ...
  const { dndContextProps, overColumn, activeId } = useBoardDnd({
    dropKeys,
    onDrop: handleDrop,
  });
      ...
      <ModuleLayout moduleContext="tasks">
        <div className="mx-auto max-w-md px-6 py-20 text-center">
          ...
          <Button asChild className="mt-6" variant="outline">
            <Link to="/kanban">К списку досок</Link>
          </Button>
        </div>
      </ModuleLayout>
    );
