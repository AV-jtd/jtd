...
- Крон пятничной рассылки Strategy deck (send-weekly-framework)
  настроен на 09:09 МСК (`9 6 * * 5`), с приветственным сообщением
  перед самой первой карточкой. Первый запуск — 2026-08-14.
- ВАЖНО, 2026-08-14: вскрыты и починены 2 системных бага storage,
  найденные через жалобу "пропал логотип Монетки в Протоколах":
  (1) S3-бэкенд бакетов (`STORAGE_BACKEND=s3`, reg.ru) падал на
  ЛЮБОЙ загрузке файлов с "Region is missing" — AWS SDK внутри
  storage-api не читал `GLOBAL_S3_REGION` для операции
  `deleteObjects`; фикс — добавлена `AWS_REGION` рядом с
  `GLOBAL_S3_REGION` в docker-compose.supabase.yml (закоммичено,
  безопасно — только ссылка на переменную, не сам секрет).
  (2) Kong вешал key-auth на ВСЕ пути `/storage/v1/`, включая
  публичные объекты — из-за этого `<img src="...">` показывал
  битую картинку (401), т.к. у HTML-тега нет способа передать
  apikey-заголовок. Фикс — в `self-hosting/kong.yml` (НЕ в git,
  секреты) добавлены два новых service-блока
  (`storage-v1-public-object`, `storage-v1-public-render`) без
  key-auth для `/storage/v1/object/public/` и
  `/storage/v1/render/image/public/`. **Если kong.yml когда-нибудь
  будет пересоздан с нуля — это надо переприменить вручную**, см.
  подробности в PROGRESS.md за 2026-08-14. Заодно перезалиты 8
  логотипов клиентов + 1 логотип протокола (все раньше указывали
  на снесённое облако Lovable, ссылки давали NXDOMAIN) — найдены
  официальные логотипы через Wikimedia Commons, каждый проверен
  визуально перед заливкой. Клиент «ФС» пропущен — внутренний,
  не публичный бренд, нужен файл от пользователя.
